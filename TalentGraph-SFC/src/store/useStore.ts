...
export const useStore = create<AppState>()(
  ...
  refreshProfile: async () => {
    ...
    let { data: profileData, error } = await supabase
      .from('users')
      .select('*, job_roles(*), user_skills(*, hard_skills(name), soft_skills(name)), certificates(*), experiences(*), educations(*)')
      .eq('id', user.id)
      .single();

    if (profileData) {
      console.log('Profile Data Fetched:', profileData.full_name);
      console.log('Job Role Data:', profileData.job_roles);
      
      // Fallback: If job_roles join failed but job_role_id exists, fetch it manually
      let jobRole = profileData.job_roles;
      if (Array.isArray(jobRole)) jobRole = jobRole[0];

      if (!jobRole && profileData.job_role_id) {
        const { data: roleData } = await supabase
          .from('job_roles')
          .select('*')
          .eq('id', profileData.job_role_id)
          .single();
        if (roleData) {
          jobRole = roleData;
        }
      ...
      // Fallback 2: If jobRole still null, try to match by target_role name
      if (!jobRole && profileData.target_role) {
        const { data: roleData } = await supabase
          .from('job_roles')
          .select('*')
          .ilike('name', profileData.target_role)
          .limit(1)
          .maybeSingle();
        if (roleData) {
          jobRole = roleData;
      ...
      set({ 
        ...
        profile: {
          ...
          email: user.email,
          fullName: profileData.full_name || '',
          targetRole: jobRole?.name || profileData.target_role || '',
          jobRoleId: profileData.job_role_id,
          salary_min: jobRole?.salary_min || jobRole?.salary || 0,
          salary_max: jobRole?.salary_max || jobRole?.salary || 0,
          salary_avg: ((Number(jobRole?.salary_min || jobRole?.salary || 0)) + (Number(jobRole?.salary_max || jobRole?.salary || 0))) / 2,
          salary_currency: jobRole?.salary_currency || 'USD',
  ...
  initializeAuth: async () => {
    ...
    try {
      ...
      if (session?.user) {
        ...
        if (profileData) {
          console.log('Auth Init Profile Data:', profileData.full_name);
          console.log('Auth Init Job Role Data:', profileData.job_roles);
  ...
  runGapAnalysis: async () => {
    ...
    await Promise.all([
      supabase.from('career_gaps').delete().eq('user_id', user.id),
      supabase.from('roadmaps').delete().eq('user_id', user.id)
    ]);
    ...
    const roadmapSteps = analysis.map((item, index) => ({
      user_id: user.id,
      title: `Master ${item.name}`,
      status: index === 0 ? 'current' : 'locked',
      duration: item.priority === 3 ? '4-6 weeks' : item.priority === 2 ? '2-3 weeks' : '1 week',
      description: `Deep dive into ${item.name} to bridge your ${item.gap}% gap and meet ${selectedJobRole.name} requirements.`,
      milestones: [`Complete ${item.name} fundamentals`, `Build a project using ${item.name}`, `Pass ${item.name} assessment`],
      point_score: item.priority * 500,
    ...
    set({ 
      gapAnalysis: analysis.map(a => ({
        ...a,
        priority: a.priority === 3 ? 'High' : a.priority === 2 ? 'Medium' : 'Low'
      })),
      roadmapData: createdRoadmap || []
    });
  },

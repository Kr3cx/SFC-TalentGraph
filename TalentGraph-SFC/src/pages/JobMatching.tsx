...
export const JobMatching: React.FC = () => {
  const hardcodedJobs: Job[] = [
    ...
    },
    {
      ...
      reason: "Your background in model safety and high-performance computing matches Anthropic's focus on constitutional AI."
    },
    {
      id: '3',
      ...
      reason: "Experience with open-source LLM optimization makes you a strong candidate for Mistral's core modeling team."
    }
  ];

  const [selectedJob, setSelectedJob] = useState<Job>(hardcodedJobs[0]);
    ...
    <div className="grid grid-cols-12 gap-8 animate-in fade-in duration-700">
      ...
      <div className="col-span-12 lg:col-span-5 space-y-4">
        <div className="flex items-center justify-between mb-4 px-2">
          <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-outline">AI Recommendations</h3>
          <span className="text-[10px] font-bold text-secondary bg-secondary/10 px-2 py-1 rounded">{hardcodedJobs.length} Matches</span>
        </div>
        ...
        {hardcodedJobs.map((job) => (
          ...
          >
            ...
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="text-right">
                <span className={cn(
                  ...
                  job.match >= 95 ? "bg-secondary/20 text-secondary" : "bg-primary/20 text-primary"
                )}>
                  {job.match}% Match
                </span>
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {job.tags.map(tag => (
                <span key={tag} className="text-[10px] font-bold bg-surface-container-highest px-2 py-1 rounded text-outline border border-white/5">
                  {tag}
                </span>
              ))}
            </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="flex justify-between items-start mb-8">
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-4 text-on-surface-variant text-sm">
                  <span className="flex items-center gap-1 font-medium"><Building2 className="w-4 h-4" /> {selectedJob.company}</span>
                  <span className="flex items-center gap-1 font-medium"><MapPin className="w-4 h-4" /> {selectedJob.location}</span>
                </div>
              </div>
              <button className="p-3 bg-surface-container-high rounded-xl text-outline hover:text-primary transition-all border border-white/5">
            ...
            <div className="bg-primary/5 border border-primary/10 rounded-2xl p-6 mb-8 flex gap-4">
              ...
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-primary mb-1">AI Match Insight</p>
                <p className="text-sm text-on-surface leading-relaxed font-medium">{selectedJob.reason}</p>
              </div>
            </div>
            ...
            <div className="space-y-8">
              <section>
                <h4 className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant mb-4">Role Overview</h4>
              ...
              <section>
                <h4 className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant mb-4">Required Neural Stack</h4>
                <div className="flex flex-wrap gap-3">
                  {selectedJob.tags.map(tag => (
                    <div key={tag} className="flex items-center gap-2 bg-surface-container-highest px-4 py-2 rounded-xl border border-white/5">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-tight text-on-surface">{tag}</span>
                    </div>
                  ))}
                </div>
              </section>
            ...
            <div className="mt-12 flex flex-col sm:flex-row gap-4">
              <button className="flex-1 bg-primary hover:bg-primary/90 text-on-primary font-black py-4 rounded-2xl shadow-xl shadow-primary/20 transition-all active:scale-95 flex items-center justify-center gap-2 uppercase tracking-widest text-xs">
                Apply Now <ArrowUpRight className="w-4 h-4" />
              </button>
              <button className="px-8 py-4 bg-surface-container-high text-on-surface font-black rounded-2xl border border-white/5 hover:bg-surface-container-highest transition-all uppercase tracking-widest text-xs">
                Refer a Friend
              </button>
            </div>
          </motion.div>

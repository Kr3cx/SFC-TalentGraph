...
export const CVBuilder: React.FC = () => {
  ...
  const jobRoles = useStore(state => state.jobRoles);
  const fetchJobRoles = useStore(state => state.fetchJobRoles);
  ...
  const handleDownloadPDF = async () => {
    ...
    if (!element || !profile) {
      setToast({ message: "Profile data not ready. Please complete your profile first.", type: 'error' });
      return;
    }

    setToast({ message: "Preparing Neural Export...", type: 'info' });

    try {
      // Ensure we are at the top of the scroll for the element
      const scrollable = element.querySelector('.no-scrollbar');
      const originalScrollTop = scrollable ? scrollable.scrollTop : 0;
      if (scrollable) scrollable.scrollTop = 0;

      // Temporarily add capture class
      ...
      const canvas = await html2canvas(element, {
        ...
        onclone: (clonedDoc) => {
          const clonedElement = clonedDoc.getElementById('cv-printable-area');
          if (clonedElement) {
            clonedElement.style.transform = 'none';
            clonedElement.style.borderRadius = '0';
            clonedElement.style.boxShadow = 'none';
            const clonedScrollable = clonedElement.querySelector('.no-scrollbar');
            if (clonedScrollable) {
              (clonedScrollable as HTMLElement).style.overflow = 'visible';
              (clonedScrollable as HTMLElement).style.height = 'auto';
            }
          }
        }
      });
      ...
      if (scrollable) scrollable.scrollTop = originalScrollTop;
      ...
      // If content is longer than one page, we scale it to fit one page for now
      // or we could implement multi-page logic. For a CV, fitting to A4 is usually preferred.
      ...
      pdf.save(`${profile.fullName.replace(/\s+/g, '_') || 'Resume'}_CV.pdf`);
      
      setToast({ message: "Neural Export Complete!", type: 'success' });
    } catch (error) {
      console.error('PDF generation error:', error);
      setToast({ message: "Export failed. Try again or use Print (Ctrl+P).", type: 'error' });
      element.classList.remove('pdf-capture');
    }
  };
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        <select 
                          value={profile.jobRoleId || ''}
                          onChange={(e) => {
                            const selectedId = e.target.value;
                            const selectedRole = jobRoles.find(r => r.id === selectedId);
                            if (selectedRole) {
                              setProfile({ 
                              ...
                              setProfile({ targetRole: e.target.value });
                            }
                          }}
                          className="w-full bg-surface-container-high border border-white/5 rounded-xl py-3 px-4 text-sm text-on-surface focus:ring-2 focus:ring-primary/50 transition-all outline-none appearance-none cursor-pointer"
                        >
                      ...
                      {profile.jobRoleId === 'other' && (
                        <input 
                          type="text" 
                          value={profile.targetRole}
                          onChange={(e) => setProfile({ targetRole: e.target.value })}
                          placeholder="Type your custom role..."
                          className="w-full mt-2 bg-surface-container-high border border-white/5 rounded-xl py-3 px-4 text-sm text-on-surface focus:ring-2 focus:ring-primary/50 transition-all outline-none animate-in slide-in-from-top-2 duration-300"
                        />
                      )}
                    </div>
                    <div className="space-y-2">
